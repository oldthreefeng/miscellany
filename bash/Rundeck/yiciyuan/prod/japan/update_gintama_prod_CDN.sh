...
user='jmr'
password='ij2hu1zh'
FTP='ftp.cdn.twgate.net'
YIWAN="gintama-japan-demo"
...
#通过FTP上传CDN资源
lftp  $user:$password@ftp.cdn.twgate.net << eof
cd /pub/japan/
mirror -R cdn/$version
eof
...
sudo rsync -avz $design_dir/  --exclude ".svn"    $cdn_dir/cdn/design/

lftp  $user:$password@ftp.cdn.twgate.net << eof
cd /pub/japan/
mirror -R -v  $cdn_dir/cdn/design/
eof
