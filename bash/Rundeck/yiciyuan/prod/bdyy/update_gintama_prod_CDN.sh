...
##### 到Ucloud CDN #######
#sudo rsync -avz cdn/$hot_version_dir  $CDN::gintama_baidu_cdn/$platform/$version
#sudo rsync -rvz -I cdn/$hot_version_dir  $CDN::gintamacdn/$platform/$version

#### 到百度CDN #######
echo '################ 上传至百度CDN  ##########################'
CDN_ZIP="$hot_version_dir.zip"
sudo zip -r $CDN_ZIP   cdn/$hot_version_dir
cd cdn/;sudo su -c 'md5sum $CDN_ZIP > upload'
