...
#同步 jidong资源到 cdn缓存目录  /data/tools/prod/baidu/baidu_cdn/jidong/1.0.6
sudo rsync -avz $cdndir/*  --exclude ".svn"    baidu_cdn/jidong/$version
...
#### 到百度CDN #######
echo '################ 上传至百度CDN  ##########################'
CDN_ZIP="jidong_$version.zip"
cd /data/tools/prod/baidu/baidu_cdn
sudo zip -r -0 -v $CDN_ZIP  jidong/$hot_version_dir
