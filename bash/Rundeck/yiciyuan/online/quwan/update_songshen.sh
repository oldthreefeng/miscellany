...
if [ -d "/data/songshen/$platform" ];then
  ...
  sudo  mkdir -p /data/songshen/$platform
  sudo chown gintama:operation /data/songshen/ -R
fi

if [ -d "/data/cdn_data/resource/$platform/$version" ];then
   echo "/data/cdn_data/resource/$platform/$version exists!"
else
   sudo mkdir -p /data/cdn_data/resource/$platform/$version
   sudo chown gintama:operation  /data/cdn_data  -R
fi
...
if [ -d "/data/gamecode/$platform" ];then
   ...
   sudo chown gintama:operation  /data/gamecode  -R
fi
...
ssh -t  wangdeyuan@101.231.68.46 "ssh gintama@10.10.41.17 'cd /data/tools/prod/$platform;sh  update_gintama_songshen.sh $platform $version'" 
