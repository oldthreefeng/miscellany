...
if [ -d "/data/gamecode/$platform/new/game_code/config/$platform/" ];then
   ...
   mkdir -p /data/gamecode/$platform/new/game_code/config/$platform
fi
...
#       ssh -t  wangdeyuan@101.231.68.46 "ssh caribbean@10.10.41.20 'cd /data/tools/prod/dream_android;sh update_jialebi_prod_CDN.sh dream_android $version;sh update_jialebi_prod_gamecode.sh dream_android $version'" 
       ssh -t  wangdeyuan@101.231.68.46 "ssh gintama@10.10.41.17 'cd /data/tools/prod/baidu;sh update_gintama_prod_gamecode.sh $platform $version'" 
...
function update_resource () {
       ...
       ssh -t  wangdeyuan@101.231.68.46 "ssh gintama@10.10.41.17 'cd /data/tools/prod/baidu;sh update_gintama_prod_CDN.sh $platform $version'" 
	...
	#IOS越狱
              sed  -i -e  "/base_version/s/$base_version_old/$hot_version_update/g" -e "/hot_version/s/$hot_version_old/$hot_version_update/g"   /data/gintama_demo_gamecode/$platform/game_code/config/bdyy/version.ini                         
	fi
                            }
