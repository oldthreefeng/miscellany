...
if [ -d "/data/gintama_demo_gamecode/$platform" ];then
   ...
   mkdir -p /data/gintama_demo_gamecode/$platform
fi
...
if [ -d "/data/gamecode/platform/new/game_code/config/jidongadr/" ];then
   echo "/data/gamecode/$platform/new/game_code/config/jidongadr  exists!"
else
   mkdir -p /data/gamecode/$platform/new/game_code/config/jidongadr
fi
...
#       ssh -t  wangdeyuan@101.231.68.46 "ssh caribbean@10.10.41.20 'cd /data/tools/prod/dream_android;sh update_jialebi_prod_CDN.sh dream_android $version;sh update_jialebi_prod_gamecode.sh dream_android $version'" 
       ssh -t  wangdeyuan@101.231.68.46 "ssh gintama@10.10.41.17 'cd /data/tools/prod/baidu;sh update_gintama_prod_gamecode.sh jidongadr $version'" 

       echo -e  "\n*********************BACKUP S10000 FILES****************\n"

       ##backup s10000 code
       hot_version_old=$(grep hot_version  /data/gintama_demo_gamecode/$platform/game_code/config/jidongadr/version.ini|awk '{print $3}'|uniq)
...
function update_resource () {
       ...
       ssh -t  wangdeyuan@101.231.68.46 "ssh gintama@10.10.41.17 'cd /data/tools/prod/baidu;sh update_gintama_prod_CDN.sh jidongadr $version'" 
       
       rsync  -avz  --exclude ".svn"     /data/gamecode/$platform/new/game_code/data      /data/gintama_demo_gamecode/$platform/game_code/
   
       base_version_old=$(grep base_version /data/gintama_demo_gamecode/$platform/game_code/config/jidongadr/version.ini|awk '{print $3}')
       hot_version_old=$(grep hot_version /data/gintama_demo_gamecode/$platform/game_code/config/jidongadr/version.ini|awk '{print $3}'|uniq)

        hot_version_update=$(awk -F":" '/hot_version/{print $2}' /data/gamecode/$platform/new/game_code/hot_version.txt)
	
	echo "old hot_version is $hot_version_old"
	echo "will change to $hot_version_update"

        sed  -i -e  "/base_version/s/$base_version_old/$hot_version_update/g" -e "/hot_version/s/$hot_version_old/$hot_version_update/g"   /data/gintama_demo_gamecode/$platform/game_code/config/jidongadr/version.ini                         
                            }
