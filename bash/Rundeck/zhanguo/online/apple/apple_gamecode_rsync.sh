...
function rsync_code () {
	user=sengoku
	source=/data/zhanguo_demo_gamecode/apple/
	...
	do  
    	echo -e "###############\e[105mRsync Code Source $source\e[0m################"
    		...
    		do 
             	...
             	rsync  $testrun   -aP --exclude="merge.conf.php"  --exclude="cron/*" --exclude="include/initerioruser.php"  --exclude="data/events.php"   --exclude "data/tools_release_events.php"              --exclude "config/app.conf.php*"   --exclude "htdoc/resource/*" --exclude ".svn/" $source   sengoku@"$host"::"$dest"
