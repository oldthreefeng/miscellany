...
hot_version_file=/data/zhanguo_demo_gamecode/ucweb/hot_version.txt
for i in $(seq 1 $num)
do
    ...
    do 
             ...
             ssh $user@$host "sed  -i   -e "/hot_version/s/$hot_version_old/$hot_version_update/"  -e  "/hot_update_url/s/1.7.0.$hot_version_old/1.7.0.$hot_version_update/"  $config"
