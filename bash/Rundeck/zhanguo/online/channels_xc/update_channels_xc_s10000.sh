...
ssh -t  wangdeyuan@101.231.68.46 "ssh sengoku@10.10.41.42 'cd /data/tools/updatecode/channels_xc/;sh update_channels_xinchang_CDN_resource.sh;sh update_channels_xinchang_gamecode.sh;'" 
...
echo -e "\n**********************BACKUP S10000 CODE*****************************\n"
 ...
 rsync     -aP  --exclude "data/events.php"  --exclude "data/tools_release_events.php"     --exclude "config/app.conf.php*"  --exclude "htdoc/resource/*" --exclude ".svn/"  --exclude "include/maintenance.php"  --exclude "include/initerioruser.php"      *   hot_version.txt    /data/zhanguo_demo_gamecode/$platform/
...
gmconfig=/var/www/html/xc_channels-backend/protected/apps/zg/config/api.yml

hot_version_update=$(awk -F":" '/hot_version/{print $2}' /data/zhanguo_demo_gamecode/channels_xc/hot_version.txt)
...
php /data/zhanguo_demo_gamecode/channels_xc/tools/gen_data.php
