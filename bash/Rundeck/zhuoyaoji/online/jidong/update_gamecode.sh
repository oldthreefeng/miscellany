...
function rsync_code () {
        ...
        echo -e "###############\e[105mRsync Code Source $source\e[0m################"
                ...
                rsync  $testrun   -avz --exclude "index.php"  --exclude "xls/" --exclude ".svn/" --exclude "conf/"   --exclude "vendor/" --exclude "logs/"  $source/   $user@"$host"::"$dest"/$platform/  &>  $tmp_file
		#
		echo "#######################################       resource   #############################################"
                rsync  $testrun   -avz --delete  $source/hotupdate/   $user@"$host"::"$dest"/$platform/hotupdate/  &>  $tmp_file
...
if [ ! -z $server_num ];then
    cd /data/immortal_demo_gamecode/jidong/
    for num in $server_list
    do   
           echo -e "\nwrite server  $num to mongo -- version "
	   if [ "$test_or_run" == "test" ];then
		  echo  "------------  test !! -------------------"
	          echo  "php  public/index.php -p apple -e $environment -s $num -i version/update"
	   elif [ "$test_or_run" == "run" ];then
	         echo  "php  public/index.php -p apple -e $environment -s $num -i version/update"
		if  php  public/index.php -p apple -e $environment -s $num -i version/update | grep 'errorId' ;then
