...
ssh -p 40022 jidong@192.168.1.247 "cp /var/lib/redis/$resource_main_old /data/backup/redis/;sudo service redis$redis_port stop;sudo cp /data/new/redis/$platform.rdb /var/lib/redis/$resource_main_old;sudo service redis$redis_port start;"
  ...
  hot_version_file=/data/jialebi_demo_gamecode/dream_ios/game_server/hot_version.txt
  ...
  gmversion=$(ssh   wangdeyuan@101.231.68.46 "grep -A2 caribbean_ios $gmconfig|awk -F":" '/hotver/{print \$2}'")
  echo "$gmversion"
  ...
  ssh -t  wangdeyuan@101.231.68.46  "sudo sed  -i  "/caribbean_ios/,+2s/hotver:$gmversion/hotver:$gmversion_new/"  $gmconfig"
