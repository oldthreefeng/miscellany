...
#获取时间
		TIME=$(ssh -t  -p 40022  $user@$host  "cd /var/www/html/dream_android/game_server/data/operator/;ls  | grep '^operator_list_' | xargs -n1 -i stat -c %y {} | awk -F: '{\$NF=\"\";print \$1\":\"\$2 }' | uniq")
#生成校验列表
		ssh -t  -p 40022  $user@$host  "cd /var/www/html/dream_android/game_server/data/operator/; ls | awk '\$NF ~ /^operator_list_/{print }'| xargs -n1 -i md5sum {}   | sort -t'_' -k3 -n"  > ${check_file}
