...
class Excel():
	...
	def writeContent(self):
	 	...
	 	for page in self.pages:
			...
			if page.has_key('flied_style'):
	 		...
	 		#获取数据
	 		if '' == keyword:
	 			sql = 'select %s from it where it_serial not regexp "%s(.*)"' % (','.join(flied),'|'.join([k['keyword'] for k in self.pages[:-1] ]))
	 		else:
	...
	def save(self,filename='测试',suffix='.xlsx'):
		...
		if sys.platform == 'win32':
			self.wb.save(filename.decode('utf8').encode('gbk'))
		else:
...
if __name__ == '__main__':
	...
	config = {
			...
			'Sheets' : {
					'default_flied' : ['it_type','it_serial','it_size','it_dec','it_cpu','it_mem','it_dep','it_user','it_reason','it_detail','it_usetime','it_buytime','it_cost','it_channel'],			
						'default_flied_style' : { 
									'font': {
										'bold' : True,
										'size' : '12'
